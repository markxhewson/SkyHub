...
public class ServerSelectMenu extends Menu {
    ...
    public void setItems() {
        ...
        super.getInventory().setItem(11, Item.createItem(
                Material.GRASS,
                "&6&lOrbit",
                Chat.colorize("&7Click to connect to Orbit."))
        );

        super.getInventory().setItem(13, Item.createItem(
                Material.DIAMOND_PICKAXE,
                "&d&lAtlas",
                Chat.colorize("&7Click to connect to Atlas."))
        );

        super.getInventory().setItem(15, Item.createItem(
                Material.MYCEL,
                "&5&lMystery",
                Chat.colorize("&7Click to connect to Mystery."))
        );
    }
    ...
    public void handleClick(Player clicker, ItemStack clickedItem, int slot) {
        switch(clickedItem.getType()) {
            case GRASS:
                clicker.sendMessage("Grass");
                break;

            case MYCEL:
                clicker.sendMessage("Mystery");
                break;

            case DIAMOND_PICKAXE:
                clicker.sendMessage("Pickaxe");
                break;
